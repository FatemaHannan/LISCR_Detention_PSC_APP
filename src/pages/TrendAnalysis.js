...
export function CombinationBuilder({ rows, ageMap, typeMap, riskMap, inspectorMap, includeMou, selected: controlledSelected, onSelectedChange, vesselFilterCount, companyMap, roMap, shipTypeMap }) {
  ...
  function buildReportHtml() {
    ...
    // include the full analysis for exactly that set, matching the live view.
    if (vesselFilterCount >= 2) {
      html += buildDrillDownHtml(computeDrillDown(rows), "Selected Vessels Analysis ("+vesselFilterCount+" vessels)");
    }

    if (expandedKey) {
      const combo = combos.find(c=>c.values.join("|")===expandedKey);
      if (combo) {
        html += buildDrillDownHtml(computeDrillDown(combo.vessels), "Drill-Down — "+esc(combo.values.join(" · ")));
      }
    }
    return html;
  }
  ...
  function computeDrillDown(vessels) {
    ...
    // the combined Ship Type + Age + Port grouping (the most specific view) — tracking which
    // vessels contribute to each, so the panel can show them, not just a count.
    ...
    vessels.forEach(v => {
      ...
      (v.deficiencies||[]).filter(d=>d.detainable).forEach(d => {
        ...
        if (comboKey) bump(detCatByCombo, comboKey);
      });
    });
