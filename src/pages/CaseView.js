...
function earliestDueType(due) {
  ...
  // ism/isps/mlc are stored as "DD-MMM-YYYY - Label" text, not separate date columns
...
export default function CaseView({canEdit, canDelete, canDownload, currentUser, importedVessels=[], preSelectImo, preSelectDate, onClearPreSelect}) {
  ...
  async function loadIntelligence(imo, company, vesselOverride) {
    ...
    // Liberian flag, which will never appear in VIP or the active Fleet Roster.
    const backfillTarget = vesselOverride || sel;
    if ((vipRow || fleetRosterRow || strickenRow) && backfillTarget) {
      const updates = {};
      const companySource = vipRow?.ism_client || fleetRosterRow?.ism_client || strickenRow?.ism_client;
      if (isBlankCompany(backfillTarget.company) && companySource) updates.company = companySource;
      const roSource = vipRow?.ro || strickenRow?.ro;
      if ((!backfillTarget.ro || backfillTarget.ro === "—" || backfillTarget.ro === "") && roSource) updates.ro = roSource;
      if ((!backfillTarget.type || backfillTarget.type === "—" || backfillTarget.type === "") && strickenRow?.vessel_type) updates.type = strickenRow.vessel_type;
  ...
  return (
    <div style={{padding:"16px"}}>
      ...
      {modalVessel&&ReactDOM.createPortal((
        <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.75)",zIndex:9999,display:"flex",alignItems:"stretch",justifyContent:"flex-start"}} onClick={()=>{if(editModal){setEditModal(null);return;}setModalVessel(null);setSel(null);}}>
          ...
          {tab==="overview"&&(
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px"}}>
              <div style={{background:"var(--bg2)",border:"1px solid var(--border)",borderRadius:"10px",padding:"13px"}}>
                ...
                {[["Vessel / IMO",v.name+" · "+v.imo],["Vessel Type",typeMap[v.imo]||(v.type!=="—"?v.type:null)||intel?.stricken?.vessel_type||"—"],["Age",ageMap[v.imo]!=null?ageMap[v.imo]+" yrs":"—"],["Port",v.port||"—"],["MoU",v.mou||"—"],["Company",!isBlankCompany(v.company)?v.company:(intel?.vip?.ism_client||intel?.fleetRoster?.ism_client||intel?.stricken?.ism_client||"—")],["Regional Office",intel?.fleetRoster?.regional_office||intel?.stricken?.regional_office||"—"],["Previous Flag",intel?.vessel?.previous_flag||"—"],["FSI Case Owner",v.fsiCaseOwner||"—"],["PSC Case Owner",v.pscOwner||"—"],["Task Owners",v.taskOwners?.join(", ")||"—"],["RO / Class",v.ro||intel?.vip?.ro||intel?.stricken?.ro||"—"],["PSCO",v.psco||"—"],["Appeal",v.appeal||"—"],["CAR Status",v.carStatus||"—"],["CAR Requested Date",v.carRequestedDate||"—"],["Client Rejection",v.clientRejection||"—"],["Dispensation",v.dispensation||"—"],["Registration Date",v.regDate?fmtDate(v.regDate):"—"],["Case Status",v.caseStatus||"—"]].map(([label,value])=>(
              ...
              <div>
                ...
                {intel?.due && (
                  <div style={{background:String(intel.due.earliest_due_status||"").toLowerCase().includes("overdue")?"var(--red-bg)":"var(--bg2)",border:"1px solid "+(String(intel.due.earliest_due_status||"").toLowerCase().includes("overdue")?"#3D1A1A":"var(--border)"),borderRadius:"10px",padding:"13px",marginBottom:"10px"}}>
                    <div style={{fontSize:"14px",fontWeight:700,marginBottom:"9px",color:String(intel.due.earliest_due_status||"").toLowerCase().includes("overdue")?"var(--red2)":"var(--text)"}}>Inspection Highlights</div>
                    <div style={{display:"flex",flexDirection:"column",gap:"5px",fontSize:"13px"}}>
                      {intel.inspections?.length>0&&<div style={{display:"flex",justifyContent:"space-between"}}><span style={{color:"var(--text3)",fontWeight:700}}>Last Inspection</span><span style={{color:"var(--text2)",fontWeight:700}}>{fmtDate(intel.inspections[0].inspection_date)} — {intel.inspections[0].flag_psc||"—"}{intel.inspections[0].inspection_type?" ("+intel.inspections[0].inspection_type+")":""}</span></div>}
                      <div style={{display:"flex",justifyContent:"space-between"}}><span style={{color:"var(--text3)",fontWeight:700}}>Earliest Due{earliestDueType(intel.due)?" ("+earliestDueType(intel.due)+")":""}</span><span style={{color:String(intel.due.earliest_due_status||"").toLowerCase().includes("overdue")?"var(--red2)":"var(--text2)",fontWeight:700}}>{intel.due.earliest_due_status||"—"}{intel.due.earliest_due?" ("+fmtDate(intel.due.earliest_due)+")":""}</span></div>
                  ...
                  const buildBriefBodyHtml = ()=>{
                    return (
                      ...
                      +sec("Inspection / Survey History","<table style='border-collapse:collapse;width:100%;table-layout:fixed;'>"
                        ...
                        +pair("Findings During Last Flag Inspection?",lastFlagInsp?.num_findings>0?"Yes ("+lastFlagInsp.num_findings+")":"No",null,null,lastFlagInsp?.num_findings>0)
                        +"<tr><td style='padding:5px 8px;border:1px solid #999;font-weight:bold;background:#f4f5f7;'>Matching Deficiency/Recurring Codes</td><td colspan='3' style='padding:5px 8px;border:1px solid #999;"+(recurringDeficiencies.length>0?"color:#a30000;font-weight:bold;":"")+"'>"+recurringDefSummary+"</td></tr>"
                  ...
                  return (
                    <div id="case-brief-print-area">
                      ...
                      <div style={{background:"var(--bg2)",border:"1px solid var(--border)",borderRadius:"8px",padding:"14px",marginBottom:"12px"}}>
                        ...
                        <div style={{borderTop:"1px solid var(--border)",paddingTop:"10px",marginBottom:"10px"}}>
                          <div style={{fontSize:"13px",fontWeight:600,color:"var(--text)",marginBottom:"8px",textTransform:"uppercase",letterSpacing:".04em"}}>Matching Deficiency/Recurring Codes</div>
                          {recurringDeficiencies.length===0 ? (
                            <div style={{fontSize:"12px",color:"var(--text3)"}}>No matching/recurring deficiency codes.</div>
                          ) : (
                            <table style={{width:"100%",borderCollapse:"collapse",fontSize:"12px"}}>
                              <thead><tr>
