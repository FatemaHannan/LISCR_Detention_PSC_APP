  ...
  function printRiskReport() {
    ...
    const html = `<!DOCTYPE html><html><head><title>Pre-Boarding Risk Report — ${esc(selected.vessel)}</title>
      ...
      ${((intel?.mc?.length||0)+(intel?.pi?.length||0))===0 ? "<p style='color:#777;'>No records on file.</p>" : "<table><tr><th>Type</th><th>Date</th><th>Category</th></tr>"
        +(intel.mc||[]).map(r=>"<tr><td style='padding:5px 10px;border:1px solid #ccc;'>MC</td><td style='padding:5px 10px;border:1px solid #ccc;'>"+esc(r.incident_date)+"</td><td style='padding:5px 10px;border:1px solid #ccc;'>"+esc(r.casualty_type||"Unspecified")+"</td></tr>").join("")
        +(intel.pi||[]).map(r=>"<tr><td style='padding:5px 10px;border:1px solid #ccc;'>PI</td><td style='padding:5px 10px;border:1px solid #ccc;'>"+esc(r.incident_date)+"</td><td style='padding:5px 10px;border:1px solid #ccc;'>"+esc(r.incident_type||"Unspecified")+"</td></tr>").join("")+"</table>"}

      <div class="section-title">MLC Complaints (${intel?.mlc?.length||0})</div>
      ${!intel?.mlc?.length ? "<p style='color:#777;'>No complaints on file.</p>" : "<table><tr><th>Date</th><th>Type</th></tr>"+intel.mlc.map(r=>"<tr><td style='padding:5px 10px;border:1px solid #ccc;'>"+esc(r.reported_date)+"</td><td style='padding:5px 10px;border:1px solid #ccc;'>"+esc(r.inspection_type||r.risk_level||"—")+"</td></tr>").join("")+"</table>"}

      <div class="section-title">DPP Vetting History (${intel?.dpp?.length||0})</div>
      ${!intel?.dpp?.length ? "<p style='color:#777;'>No vetting records on file.</p>" : "<table><tr><th>Date</th><th>Risk Level</th></tr>"+intel.dpp.map(d=>"<tr><td style='padding:5px 10px;border:1px solid #ccc;'>"+esc(d.created_date)+"</td><td style='padding:5px 10px;border:1px solid #ccc;'>"+esc(d.risk_level_at_time||"—")+"</td></tr>").join("")+"</table>"}

      <div class="section-title">PSC Detention Summary (${intel?.psc?.length||0})</div>
      ${!intel?.psc?.length ? "<p style='color:#777;'>No PSC summary records on file.</p>" : "<table><tr><th>Date</th><th>Port</th><th>Findings</th><th>Detained?</th><th>Risk</th></tr>"+intel.psc.map(p=>"<tr><td style='padding:5px 10px;border:1px solid #ccc;'>"+esc(p.inspection_date)+"</td><td style='padding:5px 10px;border:1px solid #ccc;'>"+esc(p.port||"—")+"</td><td style='padding:5px 10px;border:1px solid #ccc;'>"+esc(p.num_findings||0)+"</td><td style='padding:5px 10px;border:1px solid #ccc;'>"+(p.was_detained?"Yes":"—")+"</td><td style='padding:5px 10px;border:1px solid #ccc;'>"+esc(p.risk_level||"—")+"</td></tr>").join("")+"</table>"}
      </body></html>`;
